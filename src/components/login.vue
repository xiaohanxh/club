<template>
  <div :class="[pc?'pclogin_box':'mblogin_box']">
    <div class="login_box2">
      <div class="login"
           v-bind="userdata">
        <h1>Login</h1>
        <i class="iconfont icon-icon-test20"></i>
        <input v-model="userdata.id"
               placeholder="Username"
               @keyup.enter="logina"><br>
        ...
        <div class="bts">
          <button @click="logina"
                  @keyup.enter="logina">Login</button>
          <!-- <button>重置</button> -->
        </div>
      </div>
      <div class="login_register">
        <router-link to="/users/register">register</router-link>
        <router-link to="/users/login">Forgot password?</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    logina () {
      const that = this
      if (that.userdata.username === '' || that.userdata.password === '') {
        alert('账号或密码不能为空')
      } else {
        that.$http.post('login', that.userdata).then(res => {
          if (res.data.code === '0') {
